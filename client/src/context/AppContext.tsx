...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load user data from MongoDB API on app start
  useEffect(() => {
    const loadUserData = async () => {
      try {
        const savedUserId = localStorage.getItem('roleswap-current-user');
        const savedTheme = localStorage.getItem('roleswap-theme');
        if (savedTheme === 'dark') {
          dispatch({ type: 'TOGGLE_DARK_MODE' });
        }
        if (savedUserId) {
          const user = await database.getUserById(savedUserId);
          if (user) {
            ...
            localStorage.removeItem('roleswap-current-user');
  ...
  // Save user data to MongoDB when user changes
  useEffect(() => {
    const saveUserData = async () => {
      if (state.user && !state.isLoading) {
        try {
          await database.updateUser(state.user);
          localStorage.setItem('roleswap-current-user', state.user.id);
        } catch (error) {
